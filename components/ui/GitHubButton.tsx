'use client';

import { motion } from 'framer-motion';

interface GitHubButtonProps {
  name: string;
  githubUrl: string;
}

export default function GitHubButton({ name, githubUrl }: GitHubButtonProps) {
  const handleClick = () => {
    window.open(githubUrl, '_blank', 'noopener,noreferrer');
  };

  return (
    <motion.button
      onClick={handleClick}
      className="group relative flex h-12 w-12 items-center justify-start gap-2 overflow-hidden rounded bg-zinc-800 p-2 pr-6 font-bold text-white duration-700 hover:w-44 hover:bg-zinc-700"
      whileHover={{ scale: 1.02 }}
      whileTap={{ scale: 0.98 }}
      aria-label={`Visit ${name}'s GitHub profile`}
    >
      {/* Rotating square decoration */}
      <span className="absolute left-8 h-6 w-6 rotate-45 bg-zinc-800 duration-700 group-hover:left-40 group-hover:bg-zinc-700 z-0" />

      {/* GitHub SVG Icon */}
      <svg
        xmlns="http://www.w3.org/2000/svg"
        viewBox="0 0 24 24"
        fill="currentColor"
        className="h-8 w-8 shrink-0 text-white relative z-10"
        aria-hidden="true"
      >
        <path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z"/>
      </svg>

      {/* Expanding name text */}
      <span className="relative z-10 inline-flex origin-left transform border-l-2 border-white pl-2 text-white font-bold text-sm whitespace-nowrap scale-x-0 opacity-0 transition-all duration-300 group-hover:scale-x-100 group-hover:opacity-100 group-hover:delay-200">
        {name}
      </span>
    </motion.button>
  );
}
